...
router.get("/:id", (req, res) => {
    try {
        const factura = database_1.default
            .prepare("SELECT *, c.name as cliente_name FROM facturas f JOIN clients c ON f.cliente_id = c.id WHERE f.id = ?")
            .get(req.params.id);
        if (factura) {
            const cobros = database_1.default
      ...
      `)
                .all(req.params.id);
            res.json(Object.assign(Object.assign({}, factura), { cobros, ots }));
        }
        else {
...
router.post("/", (req, res) => {
    ...
    const transaction = database_1.default.transaction(() => {
        const stmt = database_1.default.prepare("INSERT INTO facturas (numero_factura, monto, vencimiento, estado, cliente_id) VALUES (?, ?, ?, 'pendiente', ?)");
...
router.post("/:id/cobros", (req, res) => {
    ...
    const transaction = database_1.default.transaction(() => {
        ...
        const stmt = database_1.default.prepare("INSERT INTO cobros (factura_id, monto, medio_de_pago, fecha) VALUES (?, ?, ?, ?)");
