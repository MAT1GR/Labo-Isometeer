...
router.get("/mis-ots", verifyToken, (req, res) => {
    var _a;
    // @ts-ignore
...
// [GET] /api/ots
router.get("/", (req, res) => {
    const { role, user_id, // Se espera user_id en lugar de assigned_to
    searchTerm, clientId, assignedToId, status, authorized, } = req.query;
    try {
        let query = `
      SELECT
        ot.id, ot.custom_id, ot.product as title, ot.status, ot.authorized,
...
router.put("/:id", (req, res) => {
    const { id } = req.params;
    const _a = req.body, { activities = [], role, user_id, factura_ids = [] } = _a, otData = __rest(_a, ["activities", "role", "user_id", "factura_ids"]);
    if (!user_id) {
