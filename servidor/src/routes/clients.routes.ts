...
router.post("/bulk-import", (req: Request, res: Response) => {
  ...
  const clientInsertStmt = db.prepare(
    "INSERT OR IGNORE INTO clients (code, name, client_number, fiscal_id_type, fiscal_id, address) VALUES (?, ?, ?, ?, ?, ?)"
  );
  const findClientStmt = db.prepare("SELECT id FROM clients WHERE code = ?");
  const contactInsertStmt = db.prepare(
    "INSERT INTO contacts (client_id, type, name, email, phone) VALUES (?, ?, ?, ?, ?)"
  );

  const importTransaction = db.transaction((clientList) => {
    ...
    for (const client of clientList) {
      if (client.code && client.name) {
        ...
        if (info.changes > 0) {
          ...
          const existingClient = findClientStmt.get(client.code) as
            | { id: number }
            | undefined;
          if (
            existingClient &&
